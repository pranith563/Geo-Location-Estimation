...
threshold,ret = cv2.threshold(img,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
...
def download_images_for_city(city, lat, lon):
    ...
    while num_imgs < NUM_IMAGES_PER_CITY:
        ...
        if os.path.isfile(filepath):
            ...
            thresh,ret = cv2.threshold(img,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
            size = os.path.getsize(filepath)
            if size == FAILED_DOWNLOAD_IMAGE_SIZE or thresh==threshold:
                os.remove(filepath)
                misses += 1
            else:
                num_imgs += 1
